...
export default function ApiKeysPage() {
    ...
    const handleCreate = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setNewKeyValue(res.data.keyValue); // hiển thị 1 lần duy nhất để Admin copy
            setOwnerName('');
            loadKeys();
        } catch (err) {
    ...
    return (
        <div style={{ padding: 24, maxWidth: 800, margin: '0 auto' }}>
            ...
            <form onSubmit={handleCreate} style={{ border: '1px solid #ddd', padding: 16, borderRadius: 8, marginBottom: 24 }}>
                ...
                <div style={{ marginBottom: 8 }}>
                    <label>Phạm vi (Scopes - cách nhau bởi dấu phẩy)</label><br />
                    ...
                    <tbody>
                        {keys.map((k) => (
                            <tr key={k.id} style={{ borderBottom: '1px solid #eee' }}>
                                <td>{k.ownerName}</td>
                                <td>{k.scopes}</td>
                                <td style={{ color: k.status === 'ACTIVE' ? '#15803d' : '#b91c1c' }}>
